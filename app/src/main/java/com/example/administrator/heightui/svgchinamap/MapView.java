...
//        map 的宽度  和高度
        if (totalRect != null) {
            ...
            scale= (float) (width / mapWidth);//得到一个系数
        }
        ...
        if (itemList != null) {
            ...
            for (ProviceItem proviceItem : itemList) {
                if (proviceItem != select) {
                    proviceItem.drawItem(canvas, paint, false);//开始画每一个省份
                }
            }
            if (select != null) {
...
//                中国地图的  矩形
                ...
                for (int i = 0; i < items.getLength(); i++) {
                    ...
                    Path path = PathParser.createPathFromPathData(pathData);//解析svg图片的path标签里面的android:pathData所对应的路径值
...
//                    获取每一个省份的宽高
                    ...
                    totalRect = new RectF(left, top, right, bottom);//得到一个包裹整个地图的矩形
                }
                itemList = list;
